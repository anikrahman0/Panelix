                        ...
                        <form action="" method="GET">
                            <div class="d-flex">
                                <div class="input-group">
                                    <input type="text" name="search" class="form-control post-wrapper dbcategory" value="{{ request('search') }}" placeholder="Search customer" aria-describedby="button-addon2">
                                    <button type="submit" class="btn search-list-btn" id="button-addon2"><i class="fa-solid fa-magnifying-glass"></i> </button>
                                </div>
                                <div class="">
                                    <a href="{{ route('admin.users.index-user') }}" class="btn btn-sm reset-btn"><i class="fa-solid fa-rotate"></i></a>
                                </div>
                            </div>
                        </form>
                    </div>
                        ...
                        <table class="table display responsive nowrap" width="100%" id="basicTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{$user->name}}</td>
                                    <td>{{$user->email}}</td>
                                    <td>{{ optional($user->role)->type != 1 ? 'Customer':'' }}</td>
                                    <td>
                                        @if($user->status == 1)
                                    ...
                                    <td>
                                        ...
                                        <x-action-buttons.delete
                                            :url="route('admin.users.delete-user', $user->id)" 
                                            :isSuper="auth()->guard('admin')->user()->is_super == 1" 
                                            confirmationMessage="Do you really want to delete this user?"
                                            permission="users-delete"
                                        />
                                        <x-action-buttons.password
                                            :url="route('admin.users.customer.change-password', $user->id)" 
                                            :isSuper="auth()->guard('admin')->user()->is_super == 1" 
                                            permission="users-update"
                                        />
                                    </td>
